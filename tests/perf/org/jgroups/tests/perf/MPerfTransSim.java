 ...
 * @since 3.1
 */
public class MPerfTransSim extends ReceiverAdapter {
    protected String                props;
    protected JChannel              channel;
    final protected AckCollector    ack_collector=new AckCollector(); // for synchronous sends
    protected Address               local_addr=null;
    protected String                name;
    protected int                   msgsPerSecond;
    protected int                   duration;

    protected int                   num_msgs=1000;//1000 * 1000;
    ...
    protected static final short                  ID=ClassConfigurator.getProtocolId(MPerfTransSim.class);
    ...
    public void start(String props, String name, int msgsPerSecond, int duration) throws Exception {
        this.props=props;
        this.name=name;
        this.msgsPerSecond = msgsPerSecond;
        this.duration = duration;
        this.num_msgs = duration * 60 * msgsPerSecond;
        this.log_interval=num_msgs / 10; // log every 10%
        this.receive_log_interval=num_msgs / 10;
   ...
   protected static InputStream findFile(String filename) {
        ...
        return Util.getResourceAsStream(name, MPerfTransSim.class);
    }
        ...
        Thread thread=new Thread() {
            public void run() {
                try {
                    ...
                    channel=ch;
                    channel.setName(name);
                    channel.setReceiver(MPerfTransSim.this);
                    channel.connect("mperf");
                    local_addr=channel.getAddress();
...
//                    Message msg=new Message(new AnycastAddress(destinations), payload).putHeader(ID, new MPerfHeader(MPerfHeader.DATA, new_seqno));
                    ...
                    double calc = 2.0 * (1000.0 / msgsPerSecond);
                    calc = Math.random() * calc;
            ...
            System.out.println("Duration := " + TimeUnit.MINUTES.convert(System.nanoTime() - startTime, TimeUnit.NANOSECONDS) + " minutes");
            System.out.println("Average Send rate in nano := " + (average / num_msgs));
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int msgsPerSecond = 0;
        int duration = 0; // Duration of the experiment in minutes
        ...
        for(int i=0; i < args.length; i++) {
            ...
            if("-mps".equals(args[i])) {
                msgsPerSecond = Integer.parseInt(args[++i]);
                continue;
            }
            if("-mins".equals(args[i])) {
        ...
        final MPerfTransSim test=new MPerfTransSim();
        try {
            test.start(props, name, msgsPerSecond, duration);
