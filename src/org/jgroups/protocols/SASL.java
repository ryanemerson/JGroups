...
public class SASL extends Protocol {
            ...
            if (needsAuthentication(gmsHeader)) {
                ...
                switch (saslHeader.getType()) {
                    case CHALLENGE:
                        try {
                            if (log.isTraceEnabled())
                                log.trace(String.format("%s: received CHALLENGE from %s", getAddress(), remoteAddress));
                            ...
                            if (response != null) {
                                if (log.isTraceEnabled())
                                    log.trace(String.format("%s: sending RESPONSE to %s", getAddress(), remoteAddress));
                                ...
                                if (log.isTraceEnabled())
                                    log.trace(String.format("%s: authentication complete from %s", getAddress(), remoteAddress));
                            }
                    ...
                    case RESPONSE:
                        try {
                            if (log.isTraceEnabled())
                                log.trace(String.format("%s: received RESPONSE from %s", getAddress(), remoteAddress));
                            ...
                            if (challenge != null) {
                                if (log.isTraceEnabled())
                                    log.trace(String.format("%s: sending CHALLENGE to %s", getAddress(), remoteAddress));
                ...
                try {
                    ...
                    if (ctx.isSuccessful()) {
                        if (log.isDebugEnabled()) {
                            log.debug(String.format("Authentication successful for %s", ctx.getAuthorizationID()));
                        }
                        return true;
                    } else {
                        log.warn(String.format("failed to validate SaslHeader from %s, header: %s", msg.getSrc(), saslHeader));
    ...
    protected void sendJoinRejectionMessage(Address dest, String error_msg) {
        ...
        Message msg = new Message(dest).putHeader(GMS_ID, new GmsHeader(GmsHeader.JOIN_RSP)).setBuffer(Util.streamableToBuffer(joinRes));
